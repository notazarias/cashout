    ...
    <div className="min-h-screen bg-ink px-4 py-8">
      <div className="mx-auto max-w-3xl">
        <header className="mb-6 flex items-center justify-between">
          <h1 className="font-serif text-2xl font-semibold text-paper">CashOut</h1>
          <div className="flex items-center gap-3 font-sans text-sm text-sage">
            <span className="font-mono text-paper">
              {auth.status === 'account' ? auth.user.email : `guest-${auth.guestId.slice(0, 8)}`}
            </span>
            <button onClick={handleLogoutClick} className="text-sage hover:text-paper">
              Log Out
            </button>
          </div>
        </header>
        ...
        {isGuest && (
          <div className="mb-4 rounded-sm border border-brass/40 bg-felt px-4 py-3 font-sans text-sm text-paper">
