...
export function ActiveSessionScreen() {
  ...
  if (loading) {
    return <p className="font-sans text-sm text-sage">Loading session…</p>
  }
  ...
  if (closedSession) {
    ...
    return (
      <Card>
        <h1 className="mb-1 font-serif text-2xl font-semibold text-paper">Session Complete</h1>
        ...
        <div className="mb-6 flex flex-col gap-1 font-sans text-sm text-sage">
          <span>
  ...
  return (
    <div className="flex flex-col gap-4">
      <Link to="/app" className="font-sans text-sm text-sage hover:text-paper">
        ← Dashboard
      </Link>

      {/* Game type/stakes and a visibility indicator are deliberately omitted —
          CashOut doesn't track either concept. */}
      <Card className="bg-felt">
        <div className="flex items-center gap-2">
          <span className="rounded-sm bg-brass px-1.5 py-0.5 font-mono text-[10px] font-medium text-ink">LIVE</span>
      ...
      <Card>
        <p className="mb-1 font-sans text-xs uppercase tracking-wide text-sage">In for</p>
        <Mono className="text-4xl text-paper">${(session.buyInCents / 100).toFixed(2)}</Mono>
        <div className="mt-3 flex items-center gap-4">
          <div>
            <p className="mb-1 font-sans text-xs uppercase tracking-wide text-sage">Elapsed</p>
            <ElapsedClock session={session} className="text-lg text-paper" />
          </div>
        </div>
        <p className="mt-4 font-sans text-sm text-sage">
      ...
      <Card>
        <h2 className="mb-3 font-serif text-lg font-semibold text-paper">Activity</h2>
      ...
      {showAddChips && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/80 px-4">
          <Card className="w-full max-w-sm">
            <h2 className="mb-4 font-serif text-xl font-semibold text-paper">Add Chips</h2>
            <AddBuyInForm
      ...
      {showClose && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/80 px-4">
          <Card className="w-full max-w-sm">
            <h2 className="mb-4 font-serif text-xl font-semibold text-paper">Cash Out</h2>
            <CloseSessionForm
